(function() {
    ...
    var CircleCenterSplit = [{
                "left": { "url": "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/traget-level1-split1.png", "w": "238", "h": "407", "rs": Math.PI * 2 },
                "right": { "url": "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/traget-level1-split2.png", "w": "230", "h": "407", "cutx": "26", "rs": Math.PI * 2 }
            },
            {
                "left": { "url": "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/traget-level2-split1.png", "w": "238", "h": "407", "rs": Math.PI * 2 },
                "right": { "url": "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/traget-level2-split2.png", "w": "230", "h": "407", "cutx": "26", "rs": Math.PI * 2 }
            },
            {
                "left": { "url": "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/traget-level3-split1.png", "w": "238", "h": "407", "rs": Math.PI * 2 },
                "right": { "url": "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/traget-level3-split2.png", "w": "230", "h": "407", "cutx": "26", "rs": Math.PI * 2 }
            },
        ]
        //弹夹剩余子弹数
    ...
    //是否向上震动
    var isup = true;
    var arrayLipstick = ["https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/bullet.png", "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/bullet.png", "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/bullet.png"];
    //中心圆资源数组
    var arrayCenterCircle = ["https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/target-level1.png", "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/target-level2.png", "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/target-level3.png"];
    //关卡资源
    var levelUrl = [
            { "normal": "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/level1-icon-empty.png", "active": "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/level1-icon.png" },
            { "normal": "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/level2-icon-empty.png", "active": "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/level2-icon.png" },
            { "normal": "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/level3-icon-empty.png", "active": "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/level3-icon.png" }
        ]
    ...
    //子弹宽高
    var AimBullet = { w: 66 * ratio, h: 168 * ratio }
    var AimBulletBottom = CANVAS_HEIGHT - 58 * ratio - AimBullet.h + BULLET_PIERCED_DEEP;

    var D_Json;
    var domain = 'https://idolsk-test.jammyfm.com';
    if (location.host == 'idolsk.jammyfm.com') {
        domain = 'https://idolsk.jammyfm.com';
    }
    var t = Math.random();
    $.ajax({
        url: domain + '/api/sk/game/config?' + t,
        async: false,
    ...
    var BULLET_SPEED = -30;
    //弹夹宽、高、间距、bottom值
    var CLIPSIZE = { w: 66 * ratio, h: 168 * ratio, space: 41 * ratio, bottom: 60 * ratio }
        //子弹之间的距离 = 小圆半径*2+15
    ...
    CircleCenter.prototype.resetCircleCenterRotate = function(context) {
        ...
        //     $("#bulletsNum1").attr("src", "http://img.cbnic.com/hc_doudou/" + Math.floor(this.restNum / 10) + ".png")

        // } else {
        //     $("#bulletsNum2").attr("src", "")
        //     $("#bulletsNum2").css("display", "none")
        //     $("#bulletsNum1").attr("src", "http://img.cbnic.com/hc_doudou/" + this.restNum + ".png")
        // }
    };
    ...
    Game.prototype = {
        ...
        /*游戏当前关卡*/
        isPause: false,
        /*游戏当前是否处于暂停状态, 可通过gamePause方法暂停, 只读*/
        isOver: true,
        /*当前游戏是否已经结束,只读*/
        isCanTap: true,
        /*当前游戏Canvas是否可以点击,只读*/
        ...
        init: function() {
            ...
            ballimg.src = 'https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/target-level1.png';
        ...
        last_3_changeSpeed: function(num) {
            if (num == 3) {
                ...
                if (this.rotationSpeed >= this.nowSpeed) {
                    ...
                    console.log("3rotationSpeed" + this.rotationSpeed)
                } else {
                    if (this.isFastChangeRotationAccelerationSpeed) {
                        this.rotationSpeed += params_json.rotationAccelerationSpeed;
                    } else {
                ...
                if (this.rotationSpeed >= this.nowSpeed) {
                    ...
                    console.log("2rotationSpeed" + this.rotationSpeed)
                } else {
                    if (this.isFastChangeRotationAccelerationSpeed) {
                        this.rotationSpeed += params_json.rotationAccelerationSpeed;
                    } else {
                ...
                if (this.rotationSpeed >= this.nowSpeed) {
                    ...
                    this.rotationSpeed -= params_json.rotationAccelerationSpeed;
                    console.log("1rotationSpeed" + this.rotationSpeed)
                } else {
                    if (this.isFastChangeRotationAccelerationSpeed) {
                        this.rotationSpeed += params_json.rotationAccelerationSpeed;
                    } else {
        ...
        normalResults: function() {
            if (this.bulletCircles[BulletCirclesIndex]) {
                // 灰色子弹背景
                // this.bulletCircles[BulletCirclesIndex].imgObj.src = "http://img.cbnic.com/hc_doudou/Sword_small_" + this.level + "_gray.png";
                BulletCirclesIndex++;
                ...
                this.centerCircle.restNum = parseInt(this.getParams(this.levelA).levelArray[1] - BulletCirclesIndex)
                    // 当前剩余的子弹数
                $('.bulletsNum').html("x" + this.centerCircle.restNum)
            }
            this.isCanTap = true;
        ...
        nextLevel: function() {
            ...
            if (self.level <= self.levelLength) {
                // 防止切换的时候 出现残留的上一次的等级提示
                $('#levelSwitchBoxMain').removeAttr("src");
                $("#levelSwitchBoxMain").attr("src", "https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/level_" + self.level + "_main_true.png");
                $("#levelSwitchBox").show();
        ...
        levelChange: function() {
            ...
            // 显示当前关卡数
            if (this.levelA == 0) {
                $(".level-count").html("第一关")
            }
            if (this.levelA == 1) {
                $(".level-count").html("第二关")
            }
            if (this.levelA == 2) {
                $(".level-count").html("第三关")
            }
            ...
            var ischangeSpeed = 0;
            // 显示当前关卡的倒计时时间
            $("#timebox").html(times + "s");
            var _this = this;
            // 当前第一关的时候，延迟加载后续方法，避免首次游戏的时候，关卡展示进度与倒计时并行
            if (this.levelA == 0) {
                setTimeout(function() {
                    timeboxInterval = setInterval(function() {
                        if (times <= 11 && times > 10) {
                            _this.audioPlay($("#Countdown_10s_audio").get(0))
                        }
                        ischangeSpeed++;
                        if ( /*true*/ ischangeSpeed % changeSpeed == 0) {
                            _this.oRandom = Math.floor(Math.random() * ROTAION_SPEED_ARRAY.length);
                        }
                        times--;
                        if (times == 0) {
                            _this.gameOver.call(_this);
                        }
                        $("#timebox").html(times + "s")
                        _this.player_time = times;
                    }.bind(this), 1000)
                }, 2000)
            } else {
                timeboxInterval = setInterval(function() {
            ...
            for (i = 0; i < lens; i++) {
                ...
                // 子弹背景图
                tempClipImg.src = 'https://img.jammyfm.com/media/image/songquiz_assets/mxtkgo/bullet.png';
                ...
                console.log("tempClipImg", tempClipImg)
